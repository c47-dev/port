...
- for `GlassRoundButton` or other WPF chrome with a headless harness, run `dotnet exec …/PortCheck.dll --validate-glass-round-button` and require `%TEMP%\portcheck-glass-round-validate\glass-round-button-report.txt` to contain `PASS` before delivery; exit code `0` alone is insufficient if the report file is missing
- stop every running `PortCheck.exe` (and locked `.NET Host` children) before `dotnet build`; a locked `PortCheck.exe` leaves the tray running an old binary and makes UI fixes look “broken again”
- do not treat a hung `dotnet exec` validation shell (exit `4294967295` / no report file) as a product failure until the harness path is confirmed: validation must run before tray/Docker init (`OnStartup` early exit, skip DI in `App` ctor when the flag is present)
- manual hover QA for round chrome must confirm visible scale (~1.14×) and lens/specular layers; “no visible difference” on hover often means the interaction animator never attached because `Template.FindName` failed (e.g. transforms nested in `TransformGroup`, or invalid `Height="42%"` on `Border` under some cultures)
